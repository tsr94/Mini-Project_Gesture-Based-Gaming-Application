...
def get_face_look_delta(face_landmarks):
    """Compute smooth camera look delta from EYE movement instead of nose."""
    ...
    # Average of both eyes (center of gaze)
    ...
    if prev_face_pos:
        ...
        dy = prev_face_pos[1] - y   # invert Y-axis for natural up/down
    ...
    sensitivity = 8.0     # slightly higher for subtle eye movement
    smooth_factor = 0.75  # higher = smoother but slower

    dx *= sensitivity * (1 - smooth_factor)
    dy *= sensitivity * (1 - smooth_factor)

    # Clamp to avoid excessive jumps
...
while cap.isOpened():
    ...
    if face_results.multi_face_landmarks:
        ...
        cv2.putText(frame, "Eye Tracking Active", (10, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)

        cv2.putText(frame, "Face Tracking Active", (10, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)

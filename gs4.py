...
def detect_movement(left_hand, right_hand):
    ...
    if left_hand:
        ...
        # Left hand movements
        if left_total_extended >= 4:  # Open palm
...
def get_hand_look_delta(right_hand):
    """Compute smooth camera look delta from right hand movement."""
    global prev_right_hand_pos
    ...
    # Palm (approx center)
    x = right_hand.landmark[9].x
    y = right_hand.landmark[9].y

    dx, dy = 0.0, 0.0
    if prev_right_hand_pos:
        dx = x - prev_right_hand_pos[0]
        dy = y - prev_right_hand_pos[1]

    prev_right_hand_pos = (x, y)
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        movement = detect_movement(left_hand, right_hand)
         ...
         # Camera Look Delta
        look_dx, look_dy = get_hand_look_delta(right_hand)
